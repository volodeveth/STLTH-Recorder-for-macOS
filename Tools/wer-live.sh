...
# the remote channel records it, and the transcript is compared word by word
# against what was actually written.
#
# Only the remote channel is measured — that is the one that travels over the
...
echo ""
echo "Що робити:"
echo "  • будь у дзвінку з Windows як співрозмовник;"
...
echo ""
echo "==> Транскрибую канал співрозмовника"
afconvert -f WAVE -d LEI16@16000 -c 1 "$SESSION/system.caf" "$OUT/remote.wav" >/dev/null
...
# not ship.
/opt/homebrew/bin/whisper-cli -m "$MODEL" -f "$OUT/remote.wav" -l uk \
  --suppress-nst --no-speech-thold 0.8 \
  -otxt -of "$OUT/remote" --no-prints >/dev/null 2>&1
...
echo "════════════════════════════════════════════════════════"
echo ""
$PYTHON Tools/wer.py "$REFERENCE" "$OUT/remote.txt"
